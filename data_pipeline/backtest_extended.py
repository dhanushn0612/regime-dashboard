...
def download_all_data(lookback_days=2800):
    print("Downloading all price data (covers Jan 2019 to present)...")
    end   = datetime.today()
    ...
    if os.path.exists(price_cache):
        ...
        short = [t for t, p in stock_data.items() if len(p) < 400]
        if short:
            print(f"  Re-downloading {len(short)} stocks with short history...")
            for ticker in short:
                try:
    ...
    print(f"  Downloading {len(UNIVERSE)}-stock universe...")
    stock_data = {}
...
def select_stocks_at_date(stock_data, nifty, regime_code, as_of, n_override=None):
    ...
    universe = {t:p for t,p in stock_data.items() if t in NIFTY200_LIQUID} if regime_code <= 2 else stock_data
...
def print_report(metrics, df):
    sep = "=" * 65
    print(f"\n{sep}")
    print("  EXTENDED BACKTEST RESULTS — Jan 2019 to Feb 2026")
    print(sep)
...
def run():
    ...
    nifty, vix, stock_data = download_all_data(lookback_days=2800)
    df      = run_walk_forward(nifty, vix, stock_data, n_months=72, fii_dii_df=fii_dii_df)
